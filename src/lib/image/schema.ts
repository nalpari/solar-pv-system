...
// S3 키는 `pvmap/{uuid}.{ext}` 형식만 허용 — DELETE 시 임의 키 삭제를 차단한다.
export const UPLOAD_KEY_PATTERN =
  /^pvmap\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(png|jpg|webp|bmp|gif)$/;
...
export const UploadImageResultSchema = z.object({
  ...
  fileName: z.string().meta({
    description: "S3 오브젝트 키 (pvmap/{uuid}.{ext}) — 삭제 시 그대로 전달",
