...
검증(04)을 통과한 입력값으로 **브라우저가 이 URL 로 리다이렉트**되어 결과 페이지를 표시한다. 앱이 fetch 로 호출하는 API 가 아니라 페이지 이동 목적지이며, `client.ts` 가 이 URL 을 합성해 반환하면 클라이언트가 `window.location` 으로 이동한다.
