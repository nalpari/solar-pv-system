...
## 0. 정정 (2026-07-30) — 먼저 읽어라

**이 문서가 전제한 파이프라인은 현재 코드에 존재하지 않는다.** 조사 결과 자체는 지우지 않고 아래로 남겨두되,
어디까지 유효한지 먼저 확인하고 읽어야 한다. 아래 §1~§6 의 라인 번호는 **전부 신뢰하지 말 것** — 그 시점 파일의 좌표다.

| | 이 문서의 전제 (2026-06-04) | 현재 코드 (2026-07-30) |
|---|---|---|
| 호출 구조 | `locateBbox` → `sharp` 크롭 → `tracePolygon` — **Gemini 2회 직렬** | **SAM 2 마스크(선택) → 단일 추론 1회** |
| 이미지 전처리 | `sharp` 디코드·북방마커 합성·PNG 재인코딩 | **`sharp` 자체가 없다.** 크롭 이미지를 그대로 base64 로 보낸다 |
| 업스트림 | `@google/genai` SDK 로 Gemini 직접 호출 | **OpenRouter Chat Completions** (`fetch`) — [설계](../plans/2026-07-27-gemini-to-openrouter-migration.md) |
| 인용 좌표 | `route.ts:390` 등 | 그 시점 파일은 293줄이었다. **`route.ts:390` 은 애초에 존재하지 않는 줄이다** |

### 무효가 된 부분

- **§1 결론 #2 / §2 흐름도·합산식 전체 / §3 표의 #2** — "Pro 호출 2회 직렬이 지배적"이라는 결론.
  호출은 1회다. 2단계 합산이라는 지연 구조 자체가 사라졌다.
- **§3 표의 #5 · #6, §5 해결책 #4** — `sharp` 이중 디코드 / PNG 무손실 재인코딩. 해당 코드가 없다.
- **§3 표의 #8** — `no_polygons` 조기반환 위치 논의. 단일 호출이라 "가장 비싼 stage2 이후"라는 구조가 없다.
- **§3 표의 #9, §5 해결책 #7** — `new GoogleGenAI` 커넥션 재사용. SDK 를 제거해 소멸했다.
- **모든 `route.ts:<n>` 참조** — 파일이 두 번(2단계→1단계 통합, OpenRouter 전환) 재작성됐다.
...
- **§3 #7 — `maxDuration` export 부재.** 다만 서버 → 업스트림 구간에 180초 `AbortController` 타임아웃이 생겼다.
- **§4 측정 공백 대부분** — 정확도 baseline(IoU) 과 입력 이미지 크기 분포는 아직 없다.

### 전환으로 이미 이행된 해결책

| 이 문서의 해결책 | 현재 상태 |
|---|---|
| #0 wall-clock 계측 | **이행됨** — `elapsedMs` 를 `[detect-roof] openrouter response` 로그에 남긴다. "stage별" 은 단일 호출이라 무의미해졌다 |
| #1 thinking 축소 | **이행됨** — `reasoning: { effort: "low", exclude: true }`. `thinkingBudget: 4096` 의 1:1 등가성은 **미검증**(계획 §9-7) |
| #3 모델 티어 전환 | **가능해졌다** — `OPENROUTER_MODEL` 변경만으로. 실행은 계획 §8 2단계, 30장 정량 게이트 통과가 조건 |
| — | **신규 관측 지표** — `usage.cost`(호출당 실비) · `resolvedModel` · `generationId`(사후 `GET /api/v1/generation?id=<id>` 로 실제 서빙 프로바이더 조회) |

지연 개선폭은 **아직 실측하지 않았다.** 수치를 인용하려면 계획 §8 1.7(로그 실측)이 먼저다.
