...
verified: { by: codex/gpt-5.6, at: 2026-08-12T00:00:00Z }
status: stable
sources:
  - id: page
    resource: src/app/page.tsx
...
| 축전지 | `hasBattery` | 기본 `hasBattery: true`. 유무만 선택 |
...
| `hasBattery` | `storageBatteryYn` / `storageBatterySelectYn` | 사용 시 `"Y"`, 미사용 시 `"N"` |
| — | `batteryItemId` | 제품 선택 UI가 없으므로 항상 `""` |
