...
- **겹친 꼭짓점은 기본적으로 개별로 움직인다.** 스냅으로 붙은 인접 지붕면들은 같은 좌표에 꼭짓점을
  여러 겹 갖게 되는데, 지붕 편집 모드에서 그냥 드래그하면 **집은 것 하나만** 움직인다 — 붙어 있던 면을
  떼어내는 조작이 더 자주 쓰인다는 판단이다.
- **Alt(Option)를 누른 채 끌면 겹친 것들이 함께 따라온다.** `VERTEX_LINK_RADIUS = 6px` 안에 겹쳐 있던
  다른 install 면의 꼭짓점이 같은 좌표로 이동해, 맞닿아 있던 면 사이가 벌어지지 않는다.
  묶음은 **폴리곤당 가장 가까운 꼭짓점 하나씩**이고, 개구(exclude)는 딸려오지 않는다.
  드래그 도중에 눌렀다 떼도 다음 move 부터 바로 반영된다.
- 묶음 전체는 Alt 여부와 무관하게 **항상 스냅 대상에서 빠진다.** 그냥 끌어 하나만 뗄 때 남겨둔 꼭짓점에
  도로 흡착되면 `SNAP_RADIUS` 안에서는 영영 뗄 수 없고, 함께 끌 때는 서로에게 붙어 제자리에 고정되기 때문이다.
  대신 한 번 놓았다 다시 집으면 묶음이 새로 계산되므로 그때는 원래 짝에도 다시 붙일 수 있다.
- 드래그가 끝나면 **실제로 좌표가 바뀐 면**의 처마 기준선이 리셋되고 그 위 모듈이 삭제된다.
  하나만 옮겼으면 그 하나만, Alt 로 함께 끌었으면 그 면들 전부가 해당된다 — 건드리지 않은 면의 배치는 보존된다.
